...
from backend.statistics.trivia.trivia import TriviaProcessor
...
class TestMinAttributeTidbit:
    def test_activity_types_with_only_zero_for_distances_are_filtered_out(self) -> None:
        ...
        workout_activity: SummaryActivity = ActivityFactory(
            type="Workout", distance=0.0
        )
        workout_activity_2: SummaryActivity = ActivityFactory(
            type="Workout", distance=0.0
        )
    ...
    def test_activity_types_are_correctly_filtered_out(self) -> None:
        ...
        run_activity: SummaryActivity = ActivityFactory(type="Run", distance=2.0)
        ride_activity: SummaryActivity = ActivityFactory(type="Ride", distance=1.0)
