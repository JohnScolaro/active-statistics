import boto3
import moto
from active_statistics.gui.tabs import general_trivia_tab
...
def test_trivia_tab(some_basic_runs_and_rides):
    ...
    general_trivia_tab.backend_processing_hook(some_basic_runs_and_rides, evm, 1)

    # Get the data out of our mock s3 to check that it was inserted.
    body = (
        s3.get_object(Bucket="athlete-data-storage", Key="1/general_trivia/table.json")[
            "Body"
        ]
        .read()
        .decode("utf-8")
