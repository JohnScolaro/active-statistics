"""
When you're working with a graph with "average speed" on the y-axis, you tend to want
different units for different activity types. Here are all my utils for converting
between them.
"""
...
def average_speed_to_kmph(average_speed_in_m_per_sec: uh._Quantity) -> Optional[float]:
    if math.isnan(average_speed_in_m_per_sec):
        ...
        return float(average_speed_in_m_per_sec / 1000 * 3600)
...
def average_speed_to_mins_per_km(
    ...
    if math.isnan(average_speed_in_m_per_sec):
        ...
        return dt.datetime(1970, 1, 1) + dt.timedelta(
            seconds=int(1000 / average_speed_in_m_per_sec)
        )
