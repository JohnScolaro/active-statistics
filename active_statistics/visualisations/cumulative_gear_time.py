...
def get_compact_activities(
    ...
    return [
        ...
        and activity.moving_time is not None
...
def plot_graph(
    ...
    date_arrays: dict[str, list[dt.datetime]] = defaultdict(list)
    moving_time_arrays: dict[str, list[float]] = defaultdict(list)
    for activity in activities:
        date_arrays[activity.gear_id].append(activity.start_date_local)
        moving_time_arrays[activity.gear_id].append(
            activity.moving_time.total_seconds()
        )

    for gear_id, time in moving_time_arrays.items():
        moving_time_arrays[gear_id] = list(itertools.accumulate(time))

    # Convert seconds to hours
    for gear_id, time in moving_time_arrays.items():
        moving_time_arrays[gear_id] = list(map(lambda x: x / 3600, time))
    ...
    for gear_id, date in sorted(
        date_arrays.items(), key=lambda t: t[1][0], reverse=True
    ):
        dd[gear_id] = go.Scatter(
            x=date,
            y=moving_time_arrays[gear_id],
            hovertemplate="<b>Hours</b>: %{y:.0f}<br>",
            mode="lines",
        )
